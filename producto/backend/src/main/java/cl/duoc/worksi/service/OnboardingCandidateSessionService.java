...
import cl.duoc.worksi.repository.OnboardingCandidateSessionRepository;
...
@Service
public class OnboardingCandidateSessionService {
    private final OnboardingCandidateSessionRepository onboardingCandidateSessionRepository;
    private final CommuneRepository communeRepository;
    private final UserService userService;
    private final BCryptPasswordEncoder passwordEncoder;

    public OnboardingCandidateSessionService(
        OnboardingCandidateSessionRepository onboardingCandidateSessionRepository,
        CommuneRepository communeRepository,
        UserService userService) {
        this.onboardingCandidateSessionRepository = onboardingCandidateSessionRepository;
        this.communeRepository = communeRepository;
        this.userService = userService;
        this.passwordEncoder = new BCryptPasswordEncoder();
    }

    public OnboardingCandidateSessionResponse openSession(OnboardingCandidateSessionRequest request) {
        ...
        String onboardingAccessToken = UUID.randomUUID().toString();
        long expiresInSeconds = 3600L;

        OnboardingCandidateSession session = new OnboardingCandidateSession();
        session.setId(onboardingId);
        ...
        session.setMiddleName(request.getMiddleName());
        session.setLastNamePaternal(request.getLastNamePaternal());
        session.setLastNameMaternal(request.getLastNameMaternal());
        session.setPhone(request.getPhone());
        session.setRut(request.getRut());
        session.setDocumentNumber(request.getDocumentNumber());
        session.setStreet(request.getStreet());
        session.setRegionId(request.getRegionId());
        session.setCommuneId(request.getCommuneId());
        session.setStatus(OnboardingCandidateSession.Status.OPEN);
        session.setExpiresAt(LocalDateTime.now().plusSeconds(expiresInSeconds));

        onboardingCandidateSessionRepository.save(session);

        return new OnboardingCandidateSessionResponse(
            onboardingId, onboardingAccessToken, "Bearer", expiresInSeconds);
    }



    private void validateRequiredFields(OnboardingCandidateSessionRequest request) {
